...
* *Example:* `02_work_experience_1.md` will only load `Achievements list at JustEatTakeaway.md`.

### 3. Two-Stage Execution Pipeline
* **Stage 1: Triage (Sequential):** The system runs `00_job_description_analysis.md`, cross-referencing the JD against skill gaps and employment risks. It prints a concise Go/No-Go recommendation to the CLI and asks the user for permission to proceed.
* **Stage 2: Document Generation (Parallel):** If the user inputs "Go", the system uses `asyncio` to execute prompts `01` through `06` concurrently to generate the CV sections and Cover Letter variations.
...
* It utilizes a 1-to-1 mapping strategy: The exact filename of the prompt maps directly to the placeholder in the Word document (e.g., the output of `01_professional_summary.md` replaces the `{{01_professional_summary}}` tag in the `.docx`).
